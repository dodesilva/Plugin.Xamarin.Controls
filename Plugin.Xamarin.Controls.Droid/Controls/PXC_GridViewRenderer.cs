...
using FormsGridView = Plugin.Xamarin.Controls.PXC_GridView;
...
using Xamarin.Forms;
using Sizes = Xamarin.Forms.Size;
using Xamarin.Forms.Platform.Android;
using System.Collections;
using Android.Util;
using System.Reflection;
using System.Collections.Specialized;
using Plugin.Xamarin.Controls.ViewCells;

[assembly: ExportRenderer(typeof(FormsGridView), typeof(PXC_GridViewRenderer))]
namespace Plugin.Xamarin.Controls.Droid.Controls
{
    #region GridViewRenderer
    public class PXC_GridViewRenderer : ViewRenderer<FormsGridView, SwipeRefreshLayout>
    {
        #region Fields
        ...
        public PXC_GridViewRenderer(Context context) : base(context)
        {
            _context = context;
            AutoPackage = false;
        }
        ...
        void UpdateGridLayout()
        {
            ...
            double rowWidth = 0;
            double heigthelem = 0;
            var enumerable = Element.ItemsSource as IList;
            // Make sure that MinItemWidth is never 0.
            if (enumerable.Count > 0 && enumerable.Count < 5)
            {
                rowWidth = Element.MinItemWidth > 0 ? Element.MinItemWidth : 75;
                rowheight = Element.RowHeight > 0 ? Element.RowHeight : 75;

                if (enumerable.Count == 2)
                    heigthelem = ((rowheight * 1) + enumerable.Count);
                else
                    heigthelem = ((rowheight * 2) + enumerable.Count);
            }
            else if (enumerable.Count > 4)
            {
                rowWidth = Element.MinItemWidth > 0 ? Element.MinItemWidth : 50;
                rowheight = Element.RowHeight > 0 ? Element.RowHeight : 50;
                if (enumerable.Count > 4 && enumerable.Count < 9)
                    heigthelem = ((rowheight * 2) + enumerable.Count);
                if (enumerable.Count > 8 && enumerable.Count < 13)
                    heigthelem = ((rowheight * 3) + enumerable.Count);
                if (enumerable.Count > 12 && enumerable.Count < 17)
                    heigthelem = ((rowheight * 4) + enumerable.Count);
                if (enumerable.Count > 16 && enumerable.Count < 21)
                    heigthelem = ((rowheight * 5) + enumerable.Count);
                if (enumerable.Count > 20 && enumerable.Count < 25)
                    heigthelem = ((rowheight * 6) + enumerable.Count);
                if (enumerable.Count > 24 && enumerable.Count < 29)
                    heigthelem = ((rowheight * 7) + enumerable.Count);
                if (enumerable.Count > 28 && enumerable.Count < 33)
                    heigthelem = ((rowheight * 8) + enumerable.Count);
                if (enumerable.Count > 32 && enumerable.Count < 37)
                    heigthelem = ((rowheight * 9) + enumerable.Count);
                if (enumerable.Count > 36 && enumerable.Count < 41)
                    heigthelem = ((rowheight * 10) + enumerable.Count);
            }
            //Get the span count.
            var spanCount = (int)Math.Max(1, Math.Floor(Element.Width / rowWidth));
            ...
            if (Element.IsHeightRequestWithValue)
            {
                Element.HeightRequest = heigthelem;
            }
    ...
    public class GridViewAdapter : RecyclerView.Adapter, Android.Views.View.IOnLongClickListener
    {
        ...
        public override RecyclerView.ViewHolder OnCreateViewHolder(ViewGroup parent, int viewType)
        {
            ...
            {
                rowWidth = Element.MinItemWidth > 0 ? Element.MinItemWidth : 75;
                rowheight = Element.RowHeight > 0 ? Element.RowHeight : 80;
            }
            else if (enumerable.Count > 4)
            {
                rowWidth = Element.MinItemWidth > 0 ? Element.MinItemWidth : 50;
                rowheight = Element.RowHeight > 0 ? Element.RowHeight : 60;
            }
            var gridViewCell = viewType == 0 ?
            Element.ItemTemplate.CreateContent() as FastGridCell :
            Element.GroupHeaderTemplate.CreateContent() as FastGridCell;
            ...
            var initialCellSize = new Sizes(rowWidth, rowheight);
            ...
            var height = viewType == 0 ? Convert.ToInt32(rowheight) :
            ...
            var width = Convert.ToInt32(rowWidth);
    ...
    public class GridViewCellContainer : ViewGroup
    {
        ...
        public GridViewCellContainer(Context context, FastGridCell viewCell, global::Android.Views.View parent, Sizes initialCellSize) : base(context)
        {
        ...
        protected override void OnLayout(bool changed, int left, int top, int right, int bottom)
        {
            using (var h = new Handler(Looper.MainLooper))
            {
                h.Post(() =>
                {
                    ...
                    var size = new Sizes(width, height);
